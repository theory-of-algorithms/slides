...
\begin{description}
  \item[Recall] that a set is just a collection of objects.
  \item[Elements] of a set have two important attributes -- they are either definitely in the set or definitely not in the set, and they are distinct so the same element can't be in there twice.
  \item[Alphabets] in the definition of Turing machines are just sets, and their elements are called symbols.
  \item[Strings] over alphabet are finite sequences of elements from the alphabet.
  \item[Sequences] have order, and in our case we just write them side-by-side in order.
\end{description}
\end{frame}
